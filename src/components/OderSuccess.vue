  ...
  <section class="wrapper">
    <div class="table-agile-info">
      <div class="panel panel-default">
        <div class="panel-heading">ĐƠN HÀNG THÀNH CÔNG</div>
        <div>
          <table
            class="table"
            ui-jq="footable"
        ...
        "sorting": {
          ...
          >
            ...
            <tbody v-for="odersuces in oderSuccess" :key="odersuces.id">
              <tr>
                <td>{{ odersuces.id }}</td>
                <td>{{ odersuces.custemerName }}</td>
                <td>{{ odersuces.custemerPhone }}</td>
                <td>{{ odersuces.custemerAddress }}</td>
                <td>{{ formatDate(odersuces.createDate) }}</td>
                <!-- <td>
                  <select name="" id="">
                ...
                <td>
                  ...
                  <button
                    ...
                    @click="isShow = !isShow"
                    @click.prevent="ViewOderDetail(odersuces.id)"
                    class="btn_del"
                  >
                    VIEW
                  </button>
...
export default {
  ...
  methods: {
    async getOderSuccess() {
      try {
        const repons = await axios.get(
          "https://localhost:44309/api/Order/OrderSuccess?pageIndex=1&pageSize=5"
        );
        this.oderSuccess = repons.data.items;
